...
public class InvocationSpec {
    ...
    private InvocationSpec(PID service, String method, Map<String, String> params, boolean datastream) {
        m_method = method;
        m_service = service;
        m_params = params;
    ...
    public String getDisseminationType(PID forWhom) {
        ...
        if (isDatastreamInvocation()) {
            ...
            return "info:fedora/" + object + "/" + service + "/" + method() + params;
        }
    }
    ...
    public InputStream invoke(FedoraClient client, PID onWhom) {
        try {
            ...
            throw new RuntimeException("Could not disseminate content for " + getDisseminationType(onWhom), e);
        }
    }

    /**
     * Build an InvocationSpec instance from the given properties. <code>info:fedora/&#42;/{SDef|&#42;}/method[?param1=value1...]</code>
     */
    public static InvocationSpec getInstance(String disseminationType) {
        ...
        if (!spec.matches("[^\\/]+\\/[^\\/]+(\\/[^\\/]+){0,1}")) {
            throw new IllegalArgumentException("Unable to parse dissemination type " + disseminationType);
        }
