...
public class ITQLQueryFactory implements QueryFactory, Constants
{
    ...
    private static final Logger logger = Logger.getLogger(FedoraOAIDriver.class.getName());
    ...
    public Date latestRecordDate(Iterator<? extends MetadataFormat> formats) throws RepositoryException
    {
        ...
        logger.info("Current date reported by Fedora is " + DateUtility.convertDateToString(current));
        return current;
    }
    ...
    /**
     * Convert the given Date to a String while also adding or subtracting a millisecond. The shift is
     * necessary because the provided dates are inclusive, whereas ITQL date operators are exclusive.
     */
    public String getExclusiveDateString(Date date, boolean isUntilDate)
    {
        if (date == null)
        {
            return null;
    ...
    public RemoteIterator<FedoraRecord> listRecords(Date from, Date until, FedoraMetadataFormat format)
    {
        ...
        // do primary query
        String primaryQuery = getListRecordsPrimaryQuery(afterUTC, beforeUTC, format.getMetadataSpec());
        File primaryFile = getCSVResults(primaryQuery);

        // do set membership query, if applicable
        File setFile = null;
        if (m_itemSetSpecPath != null && m_itemSetSpecPath.length() > 0)
        { // need
          // set
          // membership
          // info
            String setQuery = getListRecordsSetMembershipQuery(afterUTC, beforeUTC, format.getMetadataSpec());
        ...
        { // need
          // about
          // info
            String aboutQuery = getListRecordsAboutQuery(afterUTC, beforeUTC, format);
        ...
        try
        {
            ResultCombiner combiner = new ResultCombiner(primaryFile, setFile, aboutFile, true);
            return new CombinerRecordIterator(format.getPrefix(), mdDissType, aboutDissType, combiner);

        }
        catch (FileNotFoundException e)
        {
            throw new RepositoryException("Programmer error?  Query result " + "file(s) not found!");
        }
    }
    ...
    private void appendDateParts(String afterUTC, String beforeUTC, boolean alwaysSelectDate, StringBuilder out)
    {
        ...
        {
            out.append("and    $item     <" + VIEW.LAST_MODIFIED_DATE + "> $date\n");
        }

        // date constraints are optional
        if (afterUTC != null)
        {
            out.append("and    $date           <" + MULGARA.AFTER + "> '" + afterUTC + "'^^<" + RDF_XSD.DATE_TIME + "> in <#xsd>\n");
        }
        if (beforeUTC != null)
        {
            out.append("and    $date           <" + MULGARA.BEFORE + "> '" + beforeUTC + "'^^<" + RDF_XSD.DATE_TIME + "> in <#xsd>\n");
        }
    }
    ...
    public String getListRecordsPrimaryQuery(String afterUTC, String beforeUTC, InvocationSpec mdSpec)
    {
    ...
    public String getListRecordsSetMembershipQuery(String afterUTC, String beforeUTC, InvocationSpec mdSpec)
    {
        StringBuilder out = new StringBuilder();
    ...
    public String getListRecordsAboutQuery(String afterUTC, String beforeUTC, FedoraMetadataFormat format)
    {
        StringBuilder out = new StringBuilder();
        ...
        {
            out.append("and " + getDatastreamDissType(aboutSpec, "$item", "_about"));
        }
        else
        {
            out.append("and " + getServiceDissType(aboutSpec, "$item", "_about"));
        }
        appendOrder(out);
    ...
    private String getServiceDissType(InvocationSpec spec, String objectVar, String suffix)
    {
        ...
        {
            s.append(" and " + sDef + " <" + MULGARA.IS + "> <" + spec.service().toURI() + ">\n");
        }
        return s.toString();
    }

    private String getDatastreamDissType(InvocationSpec spec, String objectVar, String suffix)
    {
        StringBuilder s = new StringBuilder();
        String dissemination = "$diss" + suffix;
        s.append(objectVar + " <" + VIEW.DISSEMINATES + "> " + dissemination + "\n");
        s.append("and " + dissemination + " <" + VIEW.DISSEMINATION_TYPE + "> <" + spec.getDisseminationType() + ">\n");
        return s.toString();
    }

    public String getListSetInfoQuery(InvocationSpec setInfoSpec)
    {
        StringBuffer query = new StringBuffer();

        String setInfoDissQuery = "      $setDiss <test:noMatch> <test:noMatch>\n";
        String target = "$setDiss";
        String dissType = "";
        String commonWhereClause = "where $set <" + m_setSpec + "> $setSpec\n" + "and $set <" + m_setSpecName + "> $setName\n";
        ...
        if (setInfoSpec != null)
        {
            ...
            {
                setInfoDissQuery = getDatastreamDissType(setInfoSpec, "$set", "");
                target = "$diss";
            }
            else
            {
     ...
     */
    public String parseItemSetSpecPath(String itemSetSpecPath) throws RepositoryException
    {
        ...
        for (int i = 0; i < path.length; i++)
        {
            ...
            sb.append(path[i]);
            if (path[i].startsWith("$") && !(path[i].equals("$item") || path[i].equals("$set") || path[i].equals("$setSpec")))
            {
