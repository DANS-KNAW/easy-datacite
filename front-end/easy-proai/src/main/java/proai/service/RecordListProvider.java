...
public class RecordListProvider implements ListProvider<CachedContent> {
    ...
    public CloseableIterator<CachedContent> getList() throws CannotDisseminateFormatException, NoRecordsMatchException, NoSetHierarchyException,
            ServerException
    {
        CloseableIterator<CachedContent> iter = m_cache.getRecordsContent(m_from, m_until, m_prefix, m_set, m_identifiers);
        if (iter.hasNext())
            return iter;
    ...
    {
        CloseableIterator<String[]> iter = m_cache.getRecordsPaths(m_from, m_until, m_prefix, m_set);
        if (iter.hasNext())
            return iter;
        // else figure out why and throw the right exception
        if (!m_cache.formatExists(m_prefix)) {
