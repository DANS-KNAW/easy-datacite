...
public class SessionManager extends Thread
{
    ...
    public SessionManager(Properties props) throws ServerException
    {
        ...
        if (sec == null)
            throw new ServerException("Required property missing: " + PROP_SECONDSBETWEENREQUESTS);
    ...
    private void init(File baseDir, int secondsBetweenRequests) throws ServerException
    {
        ...
        if (dirs == null)
            throw new ServerException("Unable to create session directory: " + m_baseDir.getPath());
        if (dirs.length > 0)
        {
    ...
    /**
     * If force is false, clean up expired sessions. If force is true, clean up all sessions.
     */
    ...
    /**
     * Add a session to the map of tracked sessions. This is called by any session that has multiple
     * responses.
     */
    ...
    /**
     * Get response data from the appropriate session and return it. The resumption token encodes the
     * session id and part number. The first part is sessionid/0, the second part is sessionid/1, and so
     * on.
     */
