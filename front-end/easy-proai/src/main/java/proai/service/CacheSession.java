...
public class CacheSession<T> extends Thread implements Session
{
    ...
    public void run()
    {
        ...
        try
        {
            ...
            _manager.addSession(_sessionKey, this); // after this point, we depend on the session manager
                                                    // to clean up
    ...
    /**
     * Has the session expired? If this is true, the session will be cleaned by the reaper thread of the
     * session manager.
     */
    ...
    /**
     * Do all possible cleanup for this session. This includes signaling to its thread to stop asap,
     * waiting for it to stop, and removing any files/directories that remain. The implementation should
     * be fail-safe, as a session may be asked to clean itself more than once. Clean must *not* be called
     * from this session's thread.
     */
    public void clean()
    {
        ...
        if (sessionDir.exists())
        {
            ...
            _LOG.debug("Deleting session " + _sessionKey + " directory and all " + files.length + " files within");
    ...
    /**
     * Get the named response, wait for it, or throw any exception that has popped up while generating
     * parts.
     */
    public ResponseData getResponseData(int partNum) throws ServerException, BadResumptionTokenException
    {
        ...
        if (partNum == _lastSentPart || partNum == nextPart)
        {
            ...
            ResponseData response = new ResponseDataImpl(new CachedContentAggregate(listFile, _provider.getVerb(), _provider.getRecordCache()), token);
