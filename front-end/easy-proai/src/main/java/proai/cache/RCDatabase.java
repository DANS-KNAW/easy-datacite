...
public class RCDatabase {
    ...
    /**
     * Get the relative path to the Identify.xml file in the cache, or <code>null</code> if identifyPath is null (first update cycle hasn't run).
     */
    ...
    /**
     * Execute the given query and return the <code>long</code> value in the first column of the first row of the <code>ResultSet</code>, or zero if there are
     * no results.
     */
    ...
    /**
     * Add or update a record. This will create an rcItem for it if it doesn't exist. NOTE: Records will initially be given a NULL date. After a group of
     * records are updated, the date is set together with setUncommittedRecordDates(..)
     */
    ...
    /**
     * Copy all qualifying records from rcFailure to rcQueue. To avoid unintentional duplicates in the queue, it's important that the caller ensures the queue
