...
/**
 * An iterator around a database <code>ResultSet</code> that provides a <code>String[]</code> for each
 * row. Rows in the result set contain two values. The first value is a <code>String</code> representing
 * a relative filesystem path. The second value is a <code>long</code> representing a date. The returned
 * <code>String[]</code> for each row will have two parts: The first is the relative filesystem path and
 * the second is an ISO8601-formatted date (second precision).
 */
public class StringResultIterator implements CloseableIterator<String[]>
{
    ...
    public void close()
    {
        if (!m_closed)
        {
            ...
            logger.info("GC ran in " + ms + "ms and free memory " + "went from " + startFreeBytes + " to " + currentFreeBytes + " bytes.");
