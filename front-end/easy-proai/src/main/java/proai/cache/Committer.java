 ...
 * <p>
 * Items are added to a "commit queue" by <code>Worker</code> threads via the synchronized handoff() method. This can occur before the thread is actually
 * started. Once the thread is started, the items are removed asynchronously, then committed, until all workers have finished and ( the queue is empty or
...
public class Committer extends Thread {
    ...
    /**
     * This lock is used to ensure threadsafe access to the _lastCommitQueueSize primitive. By design, only two threads will ever be contending for it.
     */
    ...
    /**
     * Attempt to add the list of <code>QueueItems</code> to the commit queue and return immediately. This method will block until adding the list would not
     * cause the queue to exceed its capacity or the thread is finished running.
     * 
     * @return whether the handoff was successful. The handoff will only fail if the <code>Committer</code> thread has been stopped.
     */
    ...
    /**
     * Run the thread. This works in two phases. The first phase executes while workers are running and the second phase commits all remaining items unless
    ...
    /**
     * If any items are currently on the commit queue, take up to the maximum per-transaction off the queue and return them. Otherwise, return <code>null</code>
     * .
     */
    ...
    /**
     * Get the last known queue size as long as the last known size is not currently being written. The caller does not need to have a lock on the entire queue.
     */
    ...
    /**
     * Set the last known queue size as long as the last known size is not currently being read. The caller should already have a lock on the queue.
     */
