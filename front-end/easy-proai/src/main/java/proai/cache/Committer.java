...
/**
 * A thread for committing a series of <code>QueueItem</code> updates to the database.
 * <p>
 * Items are added to a "commit queue" by <code>Worker</code> threads via the synchronized handoff()
 * method. This can occur before the thread is actually started. Once the thread is started, the items
 * are removed asynchronously, then committed, until all workers have finished and ( the queue is empty
...
public class Committer extends Thread
{
    ...
    /**
     * This lock is used to ensure threadsafe access to the _lastCommitQueueSize primitive. By design,
     * only two threads will ever be contending for it.
     */
    ...
    /**
     * Construct a new committer with the given configuration. The caller is responsible for actually
     * starting the thread.
     */
    ...
    /**
     * Attempt to add the list of <code>QueueItems</code> to the commit queue and return immediately.
     * This method will block until adding the list would not cause the queue to exceed its capacity or
     * the thread is finished running.
     * 
     * @return whether the handoff was successful. The handoff will only fail if the
     *         <code>Committer</code> thread has been stopped.
     */
    protected synchronized boolean handoff(List<QueueItem> queueItems)
    {

        int toAddSize = queueItems.size();
        while (!_finishedRunning && (getLastCommitQueueSize() + toAddSize) > _maxCommitQueueSize)
        {
    ...
    /**
     * Run the thread. This works in two phases. The first phase executes while workers are running and
     * the second phase commits all remaining items unless processing is aborted.
     */
    public void run()
    {
    ...
    private void commit(List<QueueItem> items)
    {
        ...
        try
        {
            ...
            if (cacheCommitDate.getTime() < now.getTime())
            {
                ...
                _LOG.warn("Commit took longer than expected.  cacheCommitDate " + "estimate was therefore not safe.  If any "
                        + "harvest requests specifying until=null started " + "within the last " + diff + "ms., they might " + "have missed these records.");
            }
    ...
    private void updateItem(Connection conn, QueueItem item) throws Exception
    {
        ...
        {
            int oldFailCount = _db.getFailCount(conn, item.getIdentifier(), item.getMDPrefix());

            if (oldFailCount == -1)
            {
                _db.addFailure(conn, item.getIdentifier(), item.getMDPrefix(), item.getSourceInfo(), item.getFailDate(), item.getFailReason());
            }
            else
            {
                _db.updateFailure(conn, item.getIdentifier(), item.getMDPrefix(), item.getSourceInfo(), oldFailCount + 1, item.getFailDate(),
                        item.getFailReason());
            }
        }
    }
    ...
    /**
     * If any items are currently on the commit queue, take up to the maximum per-transaction off the
     * queue and return them. Otherwise, return <code>null</code>.
     */
    private List<QueueItem> getNextTransactionItems()
    {
        ...
        synchronized (_commitQueue)
        {
            ...
            else
            {
                ...
                while ((_commitQueue.size() > 0) && (nextItems.size() < _maxRecordsPerTransaction))
                {
    ...
    /**
     * Get the last known queue size as long as the last known size is not currently being written. The
     * caller does not need to have a lock on the entire queue.
     */
    ...
    /**
     * Set the last known queue size as long as the last known size is not currently being read. The
     * caller should already have a lock on the queue.
     */
