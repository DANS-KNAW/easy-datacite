...
/**
 * An simple OAIDriver for testing/demonstration purposes. The directory should contain the following files: identity.xml records/
 * item1-oai_dc-2005-01-01T08-50-44.xml sets/ abovetwo.xml abovetwo-even.xml abovetwo-odd.xml prime.xml formats/ oai_dc.txt line1: ns line2: loc
 */
public class OAIDriverImpl implements OAIDriver {
    ...
    public void init(Properties props) throws RepositoryException {
        ...
        if (baseDir == null) {
            throw new RepositoryException("Required property is not set: " + BASE_DIR_PROPERTY);
        }
        ...
        if (!m_identityFile.exists()) {
            throw new RepositoryException("Identity file does not exist: " + m_identityFile.getPath());
        }
        if (!m_recordsDir.exists()) {
            throw new RepositoryException("Records directory does not exist: " + m_recordsDir.getPath());
        }
        if (!m_setsDir.exists()) {
            throw new RepositoryException("Sets directory does not exist: " + m_setsDir.getPath());
        }
        if (!m_formatsDir.exists()) {
            throw new RepositoryException("Formats directory does not exist: " + m_formatsDir.getPath());
        }
    }
        ...
        try {
            ...
            for (int i = 0; i < names.length; i++) {
                if (names[i].endsWith(".xml")) {
                    ...
                    list.add(new SetInfoImpl(spec, new File(m_setsDir, names[i])));
                }
            }
            return list;
        }
        ...
        try {
            ...
            for (int i = 0; i < names.length; i++) {
                if (names[i].endsWith(".txt")) {
                    ...
                    BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(new File(m_formatsDir, names[i])), "UTF-8"));
        ...
        for (int i = 0; i < names.length; i++) {
            ...
            if (temp.length == 3 && temp[2].indexOf(".") != -1) {
                ...
                if (parts[1].equals(mdPrefix)) {
                    try {
                        long recDate = df.parse(temp[2].substring(0, temp[2].indexOf("."))).getTime();
                        if ((from == null || from.getTime() < recDate) && (until.getTime() >= recDate)) {
                            String itemID = "oai:example.org:" + parts[0];
                            list.add(new RecordImpl(itemID, mdPrefix, new File(m_recordsDir, names[i])));
                        }
                    }
                    catch (Exception e) {
                        System.out.println("WARNING: Ignoring unparsable filename: " + names[i]);
