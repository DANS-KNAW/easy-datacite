...
public class AccessibleToFieldFilter extends AbstractItemFieldFilter<AccessibleTo>
{
    ...
    /*
     * public boolean filterOut(ItemVO item) throws DomainException { final Set<AccessibleTo>
     * filterValues = getDesiredValues(); if (filterValues.size() == 0) throw new
     * NoFilterValuesSelectedException(); if (item instanceof FileItemVO) { final FileItemVO fileItem =
     * (FileItemVO) item; return !filterValues.contains(fileItem.getAccessibleTo()); } // TODO // else //
     * { // final FolderItemVO folderItem = (FolderItemVO) item; // for (final FolderItemAccessibleTo
     * AccessibleTo : folderItem.getAccessibleToList()) // { // if
     * (filterValues.contains(AccessibleTo.getAccessibleTo())) // return false; // } // } return true; }
     */
