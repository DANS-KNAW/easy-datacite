...
/**
 * A filter object that filters out file or folder items based on
 * a field (property) of that file or folder item.
 *
 * @author lobo
 * @param <T>
...
public interface ItemFieldFilter<FIELD> extends ItemFilter
{
	...
	/**
	 * Adds a filter value to the list of filter values. The values
	 * in the filter list are the values that are kept. If an item
	 * does not have this value it is filtered out.
	 *
	 * @param desiredValues
	...
	/**
	 * Tells if this item needs to be filtered out (removed from the list).
	 * This method cannot be placed on the ItemFilter interface, since it might
	 * be possible to write an ItemFilter that filters out items based on
	 * information of more than one item.
	 *
	 * @param item
