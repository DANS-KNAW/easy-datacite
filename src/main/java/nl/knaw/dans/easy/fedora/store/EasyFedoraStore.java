...
public class EasyFedoraStore extends FedoraDmoStore implements EasyStore
{
    ...
    {
        URL url = null;
        String spec = getFedora().getBaseURL() + "/get/" + sid + "/" + EasyFile.UNIT_ID;
        try
        {
            url = new URL(spec);
        }
    ...
    {
        final String spec = getFedora().getBaseURL() + "/get/" + sid + "/" + DescriptiveMetadataImpl.UNIT_ID;
        try
        {
            return new URL(spec);
        }
    ...
    {
        final MIMETypedStream mts = getFedora().getDatastreamAccessor().getDatastreamDissemination(sid, EasyMetadata.UNIT_ID, asOfDateTime);
        try
        {
