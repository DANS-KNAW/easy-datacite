...
public class DatasetResource extends AuthenticatedResource {
	...
	public Response getMetadata(@PathParam("sid") String sid) {
		try {
			Dataset d = Services.getDatasetService().getDataset(authenticate(),
					new DmoStoreId(sid));
	...
	public Response getDcMetadata(@PathParam("sid") String sid) {
		try {
			Dataset d = Services.getDatasetService().getDataset(authenticate(),
					new DmoStoreId(sid));
			return responseXmlOrJson(d.getEasyMetadata()
					.getDublinCoreMetadata().asXMLString());
		} catch (ObjectNotAvailableException e) {
			return notFound("Resource not available: " + sid);
		} catch (CommonSecurityException e) {
			return notAuthorized();
	...
	public Response getJumpoff(@PathParam("sid") String sid) {
		try {
			JumpoffDmo j = Services.getJumpoffService().getJumpoffDmoFor(
					authenticate(), new DmoStoreId(sid));
			if (j != null) {
	...
	public Response getJumpoffMetadata(@PathParam("sid") String sid) {
		try {
			JumpoffDmo j = Services.getJumpoffService().getJumpoffDmoFor(
					authenticate(), new DmoStoreId(sid));
			if (j != null) {
				return responseXmlOrJson(j.getJumpoffDmoMetadata()
						.asXMLString());
			} else {
	...
	public Response getFileItemMetadataWithPath(@PathParam("sid") String sid,
	...
	public Response getDcFileItemMetadataWithPath(@PathParam("sid") String sid,
			@PathParam("path") String path) {
	...
	private Response getFileItemMetadata(String sid, String path, String unitId) {
		try {
			...
			FileItem fileItem = Services.getItemService().getFileItemByPath(
					user, d, path);
	...
	public Response getFileTreeRoots(@PathParam("sid") String sid) {
		try {
			...
			List<ItemVO> items = Services.getItemService().getFilesAndFolders(
					user, d, d.getDmoStoreId(), -1, -1, null, null);
	...
	public Response getThumbnail(@PathParam("sid") String sid,
			@PathParam("thumbnailSid") String thumbnailSid) {
		try {
			EasyUser user = authenticate();
			Dataset d = Services.getDatasetService().getDataset(user,
					new DmoStoreId(sid));
	...
	public Response getData(@PathParam("sid") String sid) {
		try {
			...
			Dataset d = Services.getDatasetService().getDataset(user,
					new DmoStoreId(sid));
			List<ItemVO> rootItems = Services.getItemService()
					.getFilesAndFolders(user, d, d.getDmoStoreId(), -1, -1,
							null, null);
			...
			File zip = Services.getItemService()
					.getZippedContent(user, d, requestedItems).getZipFile();
	...
	public Response getSpecificDataWithPath(@PathParam("sid") String sid,
			@PathParam("path") String path) {
		try {
			EasyUser user = authenticate();
			Dataset d = Services.getDatasetService().getDataset(user,
					new DmoStoreId(sid));
			try {
				return Response.ok(tryToGetAsFolder(user, d, path),
	...
	private File tryToGetAsFolder(EasyUser user, Dataset d, String path)
			throws ServiceException {
		FolderItem folderItem = Services.getItemService().getFolderItemByPath(
				user, d, path);
		List<RequestedItem> requestedItems = new ArrayList<RequestedItem>();
		requestedItems.add(new RequestedItem(folderItem.getStoreId()));
		return Services.getItemService()
				.getZippedContent(user, d, requestedItems).getZipFile();
	}
	...
	public Response getSpecificFileWithId(@PathParam("sid") String sid,
			@PathParam("fileSid") String fileSid) {
		try {
			EasyUser user = authenticate();
			Dataset d = Services.getDatasetService().getDataset(user,
					new DmoStoreId(sid));
	...
	@Produces("application/zip")
	public Response getSpecificFolderWithId(@PathParam("sid") String sid,
			@PathParam("folderSid") String folderSid) {
		try {
			EasyUser user = authenticate();
			Dataset d = Services.getDatasetService().getDataset(user,
			...
			requestedItems
					.add(new RequestedItem(FOLDER_SID_PREFIX + folderSid));
			File zip = Services.getItemService()
					.getZippedContent(user, d, requestedItems).getZipFile();
			return Response.ok(zip).build();
		} catch (CommonSecurityException e) {
