...
public class AuthenticatedResource extends Resource {
	...
	protected EasyUser authenticate() throws ServiceException {
		List<String> authHeaders = getRequestHeaders().getRequestHeader(
				HttpHeaders.AUTHORIZATION);
		return authHeaders != null && !authHeaders.isEmpty() ? authenticate(authHeaders
				.get(0)) : new EasyUserAnonymous();
	}
	...
	private EasyUser authenticate(final String username, final String password)
			throws ServiceException {
		UsernamePasswordAuthentication auth = new UsernamePasswordAuthentication(
				username, password);
		Services.getUserService().authenticate(auth);
		return auth.getUser() != null ? auth.getUser()
				: new EasyUserAnonymous();
	}
