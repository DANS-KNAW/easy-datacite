...
public class ThumbnailUtil extends SimpleXmlWriter {
	 ...
	 */
	public static String getThumbnailIdsXml(EasyUser user, String datasetSid)
			throws ServiceException {
	...
	private static List<String> getThumbnailIds(EasyUser user, String datasetSid)
			throws ServiceException {
		Dataset d = Services.getDatasetService().getDataset(user,
				new DmoStoreId(datasetSid));
		List<ItemVO> rootItems = Services.getItemService().getFilesAndFolders(
	...
	private static void getThumbnailIds(Dataset d, EasyUser user,
			List<ItemVO> items, List<String> ids) throws ServiceException {
		for (ItemVO item : items) {
			if (item instanceof FolderItemVO) {
				FolderItemVO folder = (FolderItemVO) item;
				List<ItemVO> children = Services.getItemService()
						.getFilesAndFolders(user, d,
								new DmoStoreId(folder.getSid()), -1, -1, null,
								null);
