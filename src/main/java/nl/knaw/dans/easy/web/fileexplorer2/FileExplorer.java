...
public class FileExplorer extends AbstractDatasetModelPanel {
    ...
    public FileExplorer(String id, final DatasetModel datasetModel) {
        ...
        columns[1] = new AbstractColumn<Void>(new Model<String>("Name"), "name") {
			...
			public void populateItem(Item cellItem, String componentId, final IModel rowModel) {
				try {
					cellItem.add(new NamePanel(componentId, rowModel, explorer.getTree(), datasetModel, explorer.getContent(), hasAdditionalLicense, new AjaxLink<Void>("link")
					    {
							private static final long serialVersionUID = 1L;
        ...
        add(new IndicatingAjaxLink<Void>("downloadLink") {
        	...
        	public void onClick(AjaxRequestTarget target) {
				...
				if(requestedItems.size()!=0 && 
						(canAllChildrenBeRead || requestedItems.size()==(selectedFiles.size()+selectedFolders.size())) && 
						!hasAdditionalLicense && EasySession.getSessionUser().hasAcceptedGeneralConditions()) {
					// stream download directly
					try {
						final ZipFileContentWrapper zfcw = Services.getItemService().getZippedContent(EasySession.getSessionUser(), datasetModel.getObject(), requestedItems);
						...
						modalMessage.setContent(new ModalPopup(modalMessage, new StringResourceModel(MSG_TOO_MANY_FILES, this, new Model<TooManyFilesException>(e)).getObject()));
						modalMessage.show(target);
					} catch (ZipFileLengthException e) {
						logger.info("File size too large for download!", e.getMessage());
						// download can't be handled so show a message
						modalMessage.setContent(new ModalPopup(modalMessage, new StringResourceModel(MSG_ZIP_SIZE_TOLARGE, this, new Model<ZipFileLengthException>(e)).getObject()));
						modalMessage.show(target);
					} catch (ServiceRuntimeException e) {
        ...
        final DropDownChoice<AccessibleTo> accessRights = new DropDownChoice<AccessibleTo>("accessRights", new Model<AccessibleTo>(), Arrays.asList(AccessibleTo.values()), renderer);
        ...
        rightsForm.add(new IndicatingAjaxLink<Void>("applyFileRightsLink") {
        	...
        	public void onClick(AjaxRequestTarget target) {
				...
				try {
					...
					Services.getItemService().updateObjects(EasySession.getSessionUser(), datasetModel.getObject(), sidList, new UpdateInfo(newVisibleTo, newAccessibleTo, null, false), null);
				} catch (ServiceRuntimeException e) {
