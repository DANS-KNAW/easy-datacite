...
public class SolrSearchEngine implements SearchEngine
{
	...
	public SolrSearchEngine(String baseUrl, SearchBeanFactory searchBeanFactory)
			throws MalformedURLException
	{
		this(baseUrl, searchBeanFactory, true);
	}

	public SolrSearchEngine(String baseUrl,
			SearchBeanFactory searchBeanFactory, boolean multiCore)
			throws MalformedURLException
	{
	...
	protected synchronized SolrServerWrapper getServerByName(String indexName)
			throws SolrSearchEngineException
	{
		...
		if (result == null)
		{
			String coreUrl = baseUrl.getProtocol() + "://" + baseUrl.getHost()
					+ ":" + baseUrl.getPort();
			if (indexName != null && !indexName.equals("") && multiCore)
			{
				boolean pathHasTrailingSlash = baseUrl.getPath().charAt(
						baseUrl.getPath().length() - 1) == '/';
				boolean pathHasStartingSlash = baseUrl.getPath().charAt(0) == '/';
				coreUrl += (pathHasStartingSlash ? "" : "/")
						+ baseUrl.getPath() + (pathHasTrailingSlash ? "" : "/")
						+ indexName;
			}
			...
			{
				throw new SolrSearchEngineException("invalid index name "
						+ indexName, e);
			}

			LOGGER.debug("Solr: Opening new connection to "
					+ coreUrl.toString());
	...
	protected synchronized SolrServerWrapper getServerByIndex(Index index)
			throws SolrSearchEngineException
	{
		return (index == null) ? getServerByName((String) null)
				: getServerByName(index.getName());
	}

	@SuppressWarnings("unchecked")
	protected void doIndex(Collection<?> objects) throws SearchEngineException,
			SearchBeanException, SearchBeanConverterException,
			SearchBeanFactoryException
	{
		...
		for (Object object : objects)
		{
			...
			{
				SearchBeanConverter converter = getSearchBeanFactory()
						.getSearchBeanConverter(object.getClass());
				...
				if (doc.getFieldByName(SE_TYPE_FIELD_NAME) != null)
					throw new ReserverdFieldNameException(SE_TYPE_FIELD_NAME
							+ " is a reserved field for the SolrSearchEngine");

				SimpleIndexDocument copyDoc = new SimpleIndexDocument(doc);
				SimpleField<List<String>> typeField = new SimpleField<List<String>>(
						SE_TYPE_FIELD_NAME, SearchBeanUtil
								.getTypeHierarchy(object.getClass()));
		...
		for (Entry<Index, Collection<IndexDocument>> indexEntry : indexMap
				.entrySet())
		{
	...
	public void doDelete(Collection<?> objects) throws SearchEngineException,
			SearchBeanException
	{
		Map<Index, List<Field<?>>> delMap = new HashMap<Index, List<Field<?>>>(
				objects.size());
		for (Object object : objects)
		{
	...
	private void doCommit(List<SolrServerWrapper> servers)
	...
	@SuppressWarnings("unchecked")
	public SearchResult<? extends Document> searchDocuments(
			SearchRequest request) throws SearchEngineException
	{
		return (SearchResult<? extends Document>) getServerByIndex(
				request.getIndex()).search(request);
	}

	@SuppressWarnings("unchecked")
	public SearchResult<? extends Object> searchBeans(SearchRequest request)
			throws SearchEngineException
	{
			...
			for (Class<?> sbClass : filterBeans)
			{
				...
				if (sbType == null)
					throw new SearchEngineException(
							"could not retrieve search bean type for search bean: "
									+ sbClass.toString());
		...
		// do the search
		SimpleSearchResult result = getServerByIndex(request.getIndex())
				.search(request);
	...
	public void deleteBean(final Object searchBean)
			throws SearchEngineException
	{
		try
		{
	...
	public void deleteDocument(final IndexDocument indexDocument)
			throws SearchEngineException
	{
		try
		{
	...
	public void indexBean(final Object searchBean)
			throws SearchEngineException, SearchEngineException
	{
		try
		{
	...
	public void indexDocument(final IndexDocument indexDocument)
			throws SearchEngineException
	{
		try
		{
	...
	public void deleteBeans(Collection<? extends Object> searchBeans)
			throws SearchEngineException
	{
		try
		{
	...
	public void deleteDocuments(
			Collection<? extends IndexDocument> indexDocuments)
			throws SearchEngineException
	{
		try
		{
	...
	public void indexBeans(Collection<? extends Object> searchBeans)
			throws SearchEngineException
	{
		try
		{
