...
public class SearchSortPanel extends BaseSearchPanel
{
	...
	public SearchSortPanel(final String wicketId, final SearchModel model,
			final List<SortLinkConfig> sortLinkConfigs)
	{
	...
	public class SortFieldLink extends Link implements SearchResources
	{
		...
		public SortFieldLink(String id, SearchModel model,
				SortLinkConfig config)
		{
			super(id, model);
			this.config = config;
...
//			}
			...
			{
				sortTextLabel = new Label(SORT_LINK_TEXT, new ResourceModel(
						SEARCHSORTPANEL_RELEVANCE));
			} else
			{
				sortTextLabel = new Label(SORT_LINK_TEXT, new ResourceModel(
		...
		public void onClick()
		{
			...
			SimpleSortField newSortField = new SimpleSortField(
					config.getFieldName(), sortOrder, config.getSortType()
				);
		...
		public boolean isActiveSortField()
		{
			...
			if (activeSortField == null) return false;
