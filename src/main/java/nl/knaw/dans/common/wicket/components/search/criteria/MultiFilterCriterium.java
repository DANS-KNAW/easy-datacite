...
/**
 * A search criterium that adds a filter query on one or more fields. This comes in handy for an advanced
 * search. 26-7-2010: Unfortantely Solr does not support hit highlighting for filter queries, but only
 * for field queries. But field queries are not allowed for the Dismax handler. A highly recommended
 * handler for user based searches. Apparently Solr 1.5 will have a great solution for that, but by that
 * time I am already in Thailand drinking from a coconut.
 * 
 * @author lobo
 */
