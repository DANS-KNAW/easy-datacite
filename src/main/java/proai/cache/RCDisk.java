...
public class RCDisk
{
    ...
    /**
     * Get a new, unique path (relative to m_baseDir) for a file, based on the current time. If the
     * directory for the path does not yet exist, it will be created.
     */
    ...
    /**
     * Write the content of the given <code>Writable</code> to a new file and return the path of the
     * file, relative to the disk cache base directory.
     */
    public String write(Writable writable) throws ServerException
    {
        ...
        try
        {
            PrintWriter writer = new PrintWriter(new OutputStreamWriter(new FileOutputStream(new File(m_baseDir, path)), "UTF-8"));
    ...
    /**
     * Deletes path and all files created after it while pruning any empty directories that come about as
     * a result.
     */
    ...
    /**
     * This should be pretty fast. It relies on a specific number of directories and the fact that
     * File.delete() won't delete a directory if it is non-empty.
     */
                                    ...
                                    for (int hourIndex = 0; hourIndex < hourDirs.length; hourIndex++)
                                    {
                                        ...
                                        if (minuteDirs != null)
                                        {
                                            for (int minuteIndex = 0; minuteIndex < minuteDirs.length; minuteIndex++)
                                            {
                                                ...
                                                for (int i = 0; i < files.length; i++)
                                                {
                                                    String path = yearDirs[yearIndex].getName() + "/" + monthDirs[monthIndex].getName() + "/"
                                                            + dayDirs[dayIndex].getName() + "/" + hourDirs[hourIndex].getName() + "/"
                                                            + minuteDirs[minuteIndex].getName() + "/" + files[i];
                                                    writer.println(path);
                                                }
                                            }
                                        }
