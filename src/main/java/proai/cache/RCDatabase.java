...
public class RCDatabase
{
    ...
    public RCDatabase(Connection conn, DDLConverter ddlc, boolean mySQLTrickling, boolean backslashIsEscape, boolean pollingEnabled, RCDisk rcDisk)
    ...
    /**
     * Get the relative path to the Identify.xml file in the cache, or <code>null</code> if identifyPath
     * is null (first update cycle hasn't run).
     */
    ...
    public void putFormat(Connection conn, MetadataFormat format) throws ServerException
    {
        ...
        {
            stmt = getStatement(conn, false);
            String sql = "SELECT formatKey, namespaceURI, schemaLocation " + "FROM rcFormat " + "WHERE mdPrefix = " + qs(newprefix);
            ...
            if (rs.next())
            {
                ...
                if ((!uri.equals(newuri)) || (!loc.equals(newloc)))
                {
                    ...
                    sql = "UPDATE rcFormat SET namespaceURI = " + qsc(newuri) + "schemaLocation = " + qss(newloc) + "WHERE formatKey = " + key;
                    executeUpdate(stmt, sql);
                }
            }
            else
            {
                ...
                sql = "INSERT INTO rcFormat (mdPrefix, " + "namespaceURI, " + "schemaLocation) " + "VALUES (" + qsc(newprefix) + qsc(newuri) + qs(newloc) + ")";
                executeUpdate(stmt, sql);
            }
        }
        catch (SQLException se)
    ...
    public long getEarliestPollDate(Connection conn) throws ServerException
    {
        try
        {
            return getLongValue(conn, "SELECT lastPollDate " + "FROM rcFormat " + "ORDER BY lastPollDate ASC");
        }
        catch (SQLException e)
        {
    ...
    public long getLastPollDate(Connection conn, String mdPrefix) throws ServerException
    {
        try
        {
            return getLongValue(conn, "SELECT lastPollDate " + "FROM rcFormat " + "WHERE mdPrefix = " + qs(mdPrefix));
        }
        catch (SQLException e)
        {
    ...
    public void setLastPollDate(Connection conn, String mdPrefix, long lastPollDate) throws ServerException
    {
        ...
        {
            stmt = getStatement(conn, false);
            executeUpdate(stmt, "UPDATE rcFormat " + "SET lastPollDate = " + lastPollDate + " " + "WHERE mdPrefix = " + qs(mdPrefix));
        }
        catch (SQLException e)
        {
    ...
    /**
     * Execute the given query and return the <code>long</code> value in the first column of the first
     * row of the <code>ResultSet</code>, or zero if there are no results.
     */
    ...
    public List<CachedMetadataFormat> getFormats(Connection conn, String identifier) throws ServerException
    {
        ...
        try
        {
            ...
            if (identifier == null)
            {
                query = "SELECT formatKey, mdPrefix, namespaceURI, schemaLocation " + "FROM rcFormat";
            }
            else
            {
                query = "SELECT rcFormat.formatKey, rcFormat.mdPrefix, rcFormat.namespaceURI, rcFormat.schemaLocation " + "FROM rcFormat, rcItem, rcRecord "
                        + "WHERE rcItem.identifier = " + qss(identifier) + "AND rcRecord.itemKey = rcItem.itemKey "
    ...
    public void putSetInfo(Connection conn, String setSpec, String xmlPath) throws ServerException
    {
        ...
        try
        {
            ...
            if (rs.next())
            {
                ...
                executeUpdate(stmt, "UPDATE rcSet SET xmlPath = " + qss(xmlPath) + "WHERE setKey = " + setKey);
                addPrunable(stmt, xmlPathToPrune);
            }
            else
            {
                logger.info("Set " + setSpec + " is new. Adding to db.");
                executeUpdate(stmt, "INSERT INTO rcSet (setSpec, xmlPath) " + "VALUES (" + qsc(setSpec) + qs(xmlPath) + ")");
            }
        }
        catch (SQLException e)
        {
    ...
    /**
     * Add or update a record. This will create an rcItem for it if it doesn't exist. NOTE: Records will
     * initially be given a NULL date. After a group of records are updated, the date is set together
     * with setUncommittedRecordDates(..)
     */
    public void putRecord(Connection conn, ParsedRecord rec, Map<String, Integer> formatKeyMap) throws ServerException
    {
        ...
        try
        {
            ...
            rs = executeQuery(stmt, "SELECT recordKey, xmlPath " + "FROM rcRecord " + "WHERE itemKey = " + itemKey + " " + "AND formatKey = " + formatKey);
            if (rs.next())
            {
                ...
                executeUpdate(stmt, "UPDATE rcRecord SET modDate = NULL, " + "xmlPath = " + qss(xmlPath) + "WHERE recordKey = " + recordKey);
                addPrunable(stmt, xmlPathToPrune);
                ...
                for (int i = 0; i < setKeys.length; i++)
                {
                    ...
                    if (!priorSetKeys.contains(newSetKey))
                    {
                        ...
                        executeUpdate(stmt, "INSERT INTO rcMembership (setKey, recordKey) " + "VALUES (" + nsk + ", " + recordKey + ")");
                    }
                }
                ...
                while (liter.hasNext())
                {
                    ...
                    if (noLongerInSet)
                    {
                        ...
                        executeUpdate(stmt, "DELETE FROM rcMembership " + "WHERE recordKey = " + recordKey + " " + "AND setKey = " + psk);
                    }
                }
            ...
            else
            {
                // we're creating it
                rs.close();
                executeUpdate(stmt, "INSERT INTO rcRecord (itemKey, formatKey, modDate, xmlPath) " + "VALUES (" + itemKey + ", " + formatKey + ", NULL, "
                        + qs(xmlPath) + ")");
                rs = executeQuery(stmt, "SELECT recordKey from rcRecord " + "WHERE itemKey = " + itemKey + " " + "AND formatKey = " + formatKey);
                if (rs.next())
                {
                    int recordKey = rs.getInt(1);
                    rs.close();
                    ...
                    for (int i = 0; i < setKeys.length; i++)
                    {
                        executeUpdate(stmt, "INSERT INTO rcMembership (setKey, recordKey) " + "VALUES (" + setKeys[i] + ", " + recordKey + ")");
                    }
                }
                else
                {
    ...
    public String[] getRecordInfo(Connection conn, String itemID, String mdPrefix) throws ServerException
    {
        ...
        {
            stmt = getStatement(conn, false);
            rs = executeQuery(stmt, "SELECT rcRecord.xmlPath, rcRecord.modDate from rcItem, rcRecord, rcFormat " + "WHERE rcItem.identifier = " + qss(itemID)
                    + "AND rcItem.itemKey = rcRecord.itemKey " + "AND rcRecord.formatKey = rcFormat.formatKey " + "AND rcFormat.mdPrefix = " + qs(mdPrefix));
            if (rs.next())
            {
    ...
    public CloseableIterator<String[]> findRecordInfo(Connection conn, Date from, Date until, String prefix, String set) throws ServerException
    {
        ...
        try
        {
            ...
            {
                query.append("SELECT rcRecord.xmlPath, rcRecord.modDate FROM rcMembership, rcRecord " + "WHERE rcMembership.setKey = " + setKey + " "
                        + "AND rcMembership.recordKey = rcRecord.recordKey " + "AND rcRecord.formatKey = " + formatKey);
            }
            if (from == null)
            {
                if (until == null)
    ...
    private void createTables(Connection conn, DDLConverter ddlc) throws ServerException
    {
        ...
        catch (Exception e)
        {
            ...
            if (createdCommands.size() > 0)
            {
                ...
                {
                    msg.append("\nWARNING: An additional error occurred while " + "attempting to drop partially-created tables, "
                            + "while running the following command:\n");
                    msg.append(dropCmd);
                    String ae = ex.getClass().getName() + ": " + ex.getMessage();
                    msg.append("\nThe additional error was: " + ae);
                    msg.append("\nBefore trying again, you should manually " + "drop any remaining tables or other database objects "
    ...
    /**
     * Copy all qualifying records from rcFailure to rcQueue. To avoid unintentional duplicates in the
     * queue, it's important that the caller ensures the queue is processed beforehand.
     */
    public void queueFailedRecords(Connection conn, int maxFailedRetries) throws ServerException
    {
        ...
        if (maxFailedRetries > 0)
        {
            ...
            try
            {
                ...
                stmt = getStatement(conn, true);
                results = executeQuery(stmt, "SELECT identifier, mdPrefix, sourceInfo " + "FROM rcFailure " + "WHERE failCount <= " + maxFailedRetries);
                while (results.next())
                {
                    if (queueConn != null)
                    {
                        queueFailedRecord(queueConn, results.getString(1), results.getString(2), DBUtil.getLongString(results, 3));
                    }
                    else
                    {
                        queueFailedRecord(conn, results.getString(1), results.getString(2), DBUtil.getLongString(results, 3));
                    }
                }
            }
            catch (SQLException e)
    ...
    private void queueFailedRecord(Connection conn, String identifier, String mdPrefix, String sourceInfo) throws SQLException
    {
        ...
        try
        {
            ...
            executeUpdate(stmt, getQueueInsertSQL(identifier, mdPrefix, sourceInfo, 'F'));
        }
        finally
        {
            if (stmt != null)
    ...
    private String getQueueInsertSQL(String identifier, String mdPrefix, String sourceInfo, char queueSource)
    {
        ...
        return "INSERT INTO rcQueue (identifier, " + "mdPrefix, " + "sourceInfo, " + "queueSource) " + "VALUES (" + qsc(identifier) + qsc(mdPrefix)
                + qsc(sourceInfo) + "'" + queueSource + "')";
    }
    ...
    public void queueRemoteRecord(Connection conn, String identifier, String mdPrefix, String sourceInfo) throws ServerException
    {
        ...
        try
        {
            ...
            stmt = conn.createStatement();
            executeUpdate(stmt, getQueueInsertSQL(identifier, mdPrefix, sourceInfo, 'R'));
        }
        catch (SQLException e)
        {
    ...
    public void dumpQueue(Connection conn, PrintWriter writer) throws ServerException
    {
        ...
        try
        {
            ...
            stmt = conn.createStatement();
            results = executeQuery(stmt, "SELECT queueKey, identifier, " + "mdPrefix, sourceInfo, queueSource " + "FROM rcQueue " + "ORDER BY queueKey ASC");
            while (results.next())
            {
                resultCount++;
                ...
                if ((sourceInfo.indexOf("\n") != -1) || (sourceInfo.indexOf("\r") != -1))
                {
                    throw new ServerException("rcQueue contains bad sourceInfo for " + identifier + "/" + mdPrefix + " (contains " + "newline(s)): '"
                            + sourceInfo + "'");
                }
    ...
    public void removeFailure(Connection conn, String identifier, String mdPrefix) throws ServerException
    {
        ...
        {
            stmt = conn.createStatement();
            executeUpdate(stmt, "DELETE FROM rcFailure " + "WHERE identifier = " + qss(identifier) + "AND mdPrefix = " + qs(mdPrefix));
        }
        catch (SQLException e)
        {
    ...
    public int getFailCount(Connection conn, String identifier, String mdPrefix)
    {
        ...
        try
        {
            ...
            stmt = conn.createStatement();
            results = executeQuery(stmt, "SELECT failCount FROM rcFailure " + "WHERE identifier = " + qss(identifier) + "AND mdPrefix = " + qs(mdPrefix));
    ...
    public void addFailure(Connection conn, String identifier, String mdPrefix, String sourceInfo, String failDate, String failReason) throws ServerException
    {
        ...
        {
            stmt = conn.createStatement();
            executeUpdate(stmt, "INSERT INTO rcFailure " + "(identifier, " + "mdPrefix, " + "sourceInfo, " + "failCount, " + "firstFailDate, "
                    + "lastFailDate, " + "lastFailReason) " + "VALUES (" + qsc(identifier) + qsc(mdPrefix) + qsc(sourceInfo) + "0, " + qsc(failDate)
                    + qsc(failDate) + qs(failReason) + ")");
        }
        catch (SQLException e)
        {
    ...
    public void updateFailure(Connection conn, String identifier, String mdPrefix, String sourceInfo, int newFailCount, String failDate, String failReason)
            throws ServerException
    {
        Statement stmt = null;
        try
        {
            stmt = conn.createStatement();
            executeUpdate(stmt, "UPDATE rcFailure " + "SET sourceInfo = " + qs(sourceInfo) + ", " + "failCount = " + newFailCount + ", " + "lastFailDate = "
                    + qsc(failDate) + "lastFailReason = " + qss(failReason) + "WHERE identifier = " + qss(identifier) + "AND mdPrefix = " + qs(mdPrefix));
        }
        catch (SQLException e)
        {
    ...
    public int dumpPrunables(Connection conn, PrintWriter writer) throws ServerException
    {
        ...
        {
            stmt = conn.createStatement();
            results = executeQuery(stmt, "SELECT pruneKey, xmlPath " + "FROM rcPrunable");
            while (results.next())
            {
                resultCount++;
