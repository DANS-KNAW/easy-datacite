...
public class MPTQueryFactory implements QueryFactory, Constants
{
    ...
    private static final Logger logger = Logger.getLogger(MPTQueryFactory.class.getName());

    public static final String PROP_DDL = FedoraOAIDriver.NS + "mpt.db.ddlGenerator";

    public static final String PROP_URL = FedoraOAIDriver.NS + "mpt.jdbc.url";

    public static final String PROP_USERNAME = FedoraOAIDriver.NS + "mpt.jdbc.user";

    public static final String PROP_PASSWD = FedoraOAIDriver.NS + "mpt.jdbc.password";

    public static final String PROP_PREDICATE_MAP = FedoraOAIDriver.NS + "mpt.db.map";

    public static final String PROP_MAP_PREFIX = FedoraOAIDriver.NS + "mpt.db.prefix";

    public static final String PROP_DB_DRIVER = FedoraOAIDriver.NS + "mpt.db.driverClassName";

    public static final String PROP_BACKSLASH_ESCAPE = FedoraOAIDriver.NS + "mpt.db.backslashIsEscape";
    ...
    public Date latestRecordDate(Iterator<? extends MetadataFormat> fedoraMetadataFormats)
    {
        ...
        try
        {
            mods = adaptor.getTableFor(NTriplesUtil.parsePredicate("<info:fedora/fedora-system:def/view#lastModifiedDate>"));
        }
        ...
        try
        {
            ...
            PreparedStatement s = c.prepareStatement("SELECT max(o) FROM " + mods, ResultSet.FETCH_FORWARD, ResultSet.CONCUR_READ_ONLY);
        ...
        /*
         * Just to be on the safe side, if it's obvious we can't parse the date, then return 'now'.
         */
    ...
    public RemoteIterator<FedoraRecord> listRecords(Date from, Date until, FedoraMetadataFormat format)
    {
        ...
        try
        {
            ...
            requiredPath.addTriplePattern(getPattern("$item", "<" + Constants.VIEW.LAST_MODIFIED_DATE + ">", "$date"));

            addSpecPatterns(mdSpec, requiredPath, "$item", "_md");

            /*
             * If deletetedState isn't set, then use the default (i.e. per dissemination), else use the
             * user-defined property at a per-object basis (i.e applies for *all* datastreams)
             */
            if (deletedState.equals(""))
            {
                logger.debug("No explicit deleted state, so using state of dissemination...");
                requiredPath.addTriplePattern(getPattern("$item", "<" + Constants.MODEL.STATE + ">", "$state"));
            }
            else
            {
            ...
            /*
             * Note: We add a milisecond to the 'until', in order to assure that the comparison is
             * inclusive
             */
        ...
        List<String> targets = new LinkedList<String>(Arrays.asList("$item", "$itemID", "$date", "$state", "$setSpec"));
        ...
        GraphQuerySQLProvider builder = new GraphQuerySQLProvider(adaptor, query, backslashIsEscape);
    ...
    public RemoteIterator<SetInfo> listSetInfo(InvocationSpec setInfoSpec)
    {
        ...
        try
        {
            dissTarget = addSpecPatterns(setInfoSpec, disseminationPath, "$set", "_spec");
        }
        catch (ParseException e)
        {
        ...
        targets.addAll(Arrays.asList(new String[] {"$set", "$setSpec", "$setName"}));
        ...
        return new MPTSetInfoIterator(client, builder, dataSource, dissTarget, setInfoSpec);
    }
    ...
    private TableManager getTableManager(Properties props)
    {
        ...
        try
        {
            logger.debug("USING DRIVER " + FedoraOAIDriver.getRequired(props, PROP_DB_DRIVER));
        ...
        String escape = FedoraOAIDriver.getOptional(props, PROP_BACKSLASH_ESCAPE);
        if (escape != null)
        {
    ...
    private GraphPattern parseSetSpecPath(String itql)
    {
        ...
        /*
         * First, make sure the 'itql' arc has some semblance of correctness. Throw out obvious errors
         */
        ...
        for (int i = 0; i < path.length; i += 3)
        {
            ...
            try
            {
                setSpecPattern.addTriplePattern(getPattern(path[i], translate(path[i + 1]), path[i + 2]));
            }
            catch (ParseException e)
            {
    ...
    private static String addSpecPatterns(InvocationSpec spec, GraphPattern g, String target, String differentiator) throws ParseException
    {
        if (spec == null)
        {
            return null;
    ...
    private static void addDatastreamDissPatterns(InvocationSpec spec, GraphPattern g, String target, String differentiator) throws ParseException
    {
        String dissemination = "$diss" + differentiator;
        String disseminationType = "<info:fedora/*/" + spec.method() + ">";
        g.addTriplePattern(getPattern(target, "<" + Constants.VIEW.DISSEMINATES + ">", dissemination));
        g.addTriplePattern(getPattern(dissemination, "<" + Constants.VIEW.DISSEMINATION_TYPE + ">", disseminationType));
    }

    private static void addServiceDissPatterns(InvocationSpec spec, GraphPattern g, String target, String differentiator) throws ParseException
    {
        String model_var = "$model" + differentiator;
        String sDef_var = "$SDef" + differentiator;
        String method_var = "$method" + differentiator;
        g.addTriplePattern(getPattern(target, "<" + Constants.MODEL.HAS_MODEL + ">", model_var));
        g.addTriplePattern(getPattern(model_var, "<" + Constants.MODEL.HAS_SERVICE + ">", sDef_var));
        g.addTriplePattern(getPattern(sDef_var, "<" + Constants.MODEL.DEFINES_METHOD + ">", method_var));
    ...
    private static TriplePattern getPattern(String s, String p, String o) throws ParseException
    {
        ...
        {
            subject = new BasicNodePattern<SubjectNode>(NTriplesUtil.parseSubject(s));
        }

        predicate = new BasicNodePattern<PredicateNode>(NTriplesUtil.parsePredicate(p));
        ...
        {
            object = new BasicNodePattern<ObjectNode>(NTriplesUtil.parseObject(o));
        }

        return new BasicTriplePattern(subject, predicate, object);
    }

    private static NodeFilter<Node> getFilter(String node, String operator, String value) throws ParseException
    {
        NodePattern<Node> n;
        NodePattern<Node> v;
