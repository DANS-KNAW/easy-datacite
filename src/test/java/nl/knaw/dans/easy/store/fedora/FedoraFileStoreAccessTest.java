...
{
    private static final Logger logger = LoggerFactory.getLogger(FedoraFileStoreAccessTest.class);
    ...
    {
        assertTrue("Folders have different amount of creator roles", folder1.getCreatorRoles().size() == folder2.getCreatorRoles().size());

        assertTrue("Folders have different amount of visible to statusses", folder1.getVisibleToList().size() == folder2.getVisibleToList().size());
        ...
        for (FolderItemCreatorRole role1 : folder1.getCreatorRoles())
        {
            ...
            for (FolderItemCreatorRole role2 : folder2.getCreatorRoles())
            {
                ...
                {
                    assertTrue("Same roles found but with different sids ", role1.getFolderSid().equals(role2.getFolderSid()));
                    found = true;
                    break;
                }
            }
        ...
        for (FolderItemVisibleTo visibleTo1 : folder1.getVisibleToList())
        {
            ...
            for (FolderItemVisibleTo visibleTo2 : folder2.getVisibleToList())
            {
                ...
                {
                    assertTrue("Same visible to statusses found but with different sids ", visibleTo1.getFolderSid().equals(visibleTo2.getFolderSid()));
                    found = true;
                    break;
                }
            }

            assertTrue("Folder with sid " + folder1.getSid() + " has the visible to status " + visibleTo1.getVisibleTo().toString()
                    + " which folder2 with sid " + folder2.getSid() + " does not have ", found);
        }
        ...
        for (FolderItemAccessibleTo accessibleTo1 : folder1.getAccessibleToList())
        {
            ...
            for (FolderItemAccessibleTo accessibleTo2 : folder2.getAccessibleToList())
            {
                ...
                {
                    assertTrue("Same accessible to statusses found but with different sids ", accessibleTo1.getFolderSid().equals(accessibleTo2.getFolderSid()));
                    found = true;
                    break;
                }
            }

            assertTrue("Folder with sid " + folder1.getSid() + " has the accessibility to status " + accessibleTo1.getAccessibleTo().toString()
                    + " which folder2 with sid " + folder2.getSid() + " does not have ", found);
        }
    }
     ...
     */
    private void compareItemLists(List<? extends ItemVO> list1, List<? extends ItemVO> list2)
    {
        ...
        for (int i = 0; i < list1.size(); i++)
        {
            ...
            for (int j = 0; j < list1.size(); j++)
            {
                ...
                {
                    assertTrue("Items not of the same type", item2.getClass().getName().equals(item1.getClass().getName()));
            ...
            assertTrue("Item with sid " + list1.get(i).getSid() + " not found in list2.", found);
        }
    }
    ...
    private void insertDummyItems(String parentSid) throws StoreAccessException
    {
        ...
        List<FolderItemVO> folders = dummyFileStore.getFolders(new DmoStoreId(parentSid), -1, -1, null, null);

        // save folders to database
        insertItems(folders);

        List<FileItemVO> files = dummyFileStore.getFiles(new DmoStoreId(parentSid), -1, -1, null, null);
        insertItems(files);
    ...
    private void compareDummyToDb(DmoStoreId parentSid, boolean recursive, ItemFilters filters) throws StoreAccessException
    {
        ...
        List<ItemVO> filesAndfolders = fileStoreAccess.getFilesAndFolders(parentSid, -1, -1, null, filters);

        List<ItemVO> dummyFilesAndFolders = dummyFileStore.getFilesAndFolders(parentSid, -1, -1, null, filters);
    ...
    @Test
    public void insertSelectAndCompareItemVOs() throws RepositoryException, HibernateException, CloneNotSupportedException
    {
    ...
    public void getFilenamesTest() throws RepositoryException
    {
        ...
        List<String> filenames = fileStoreAccess.getFilenames(new DmoStoreId(DummyFileStoreAccess.DUMMY_DATASET_SID), true);

        List<String> dummyFilenames = dummyFileStore.getFilenames(new DmoStoreId(DummyFileStoreAccess.DUMMY_DATASET_SID), true);
