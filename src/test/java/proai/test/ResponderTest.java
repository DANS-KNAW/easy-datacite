...
public class ResponderTest extends TestCase
{
    ...
    private void doGetGoodRecord(String item, String prefix) throws Exception
    {
        ...
        {
            assertEquals("Failed to get " + prefix + " record of item: " + item + " (Error message was: " + e.getMessage() + ")", true, false);
        }
    }
    ...
    private void doGetBadRecord(String item, String prefix) throws Exception
    {
        ...
        {
            tryGetRecord(item, prefix);
            assertEquals("Expected failure to get " + prefix + " record for item: " + item, true, false);
        }
        catch (ServerException e)
        {
    ...
    private void tryGetRecord(String item, String prefix) throws Exception
    {
        ...
        try
        {
            ...
            if (m_print)
                printResult("getRecord(" + item + ", " + prefix + ")", data);
        }
        finally
        {
            if (data != null)
    ...
    private void doListMetadataFormatsTest(String item) throws Exception
    {
        ...
        try
        {
            ...
            if (m_print)
                printResult("listMetadataFormats(" + item + ")", data);
        }
        finally
        {
            if (data != null)
    ...
    private void doListSetsTest() throws Exception
    {
        ...
        try
        {
            ...
            if (m_print)
                printResult("listSets(null)", data);
            ...
            while (token != null)
            {
                ...
                if (m_print)
                    printResult("listSets(" + token + ")", data);
                data = m_responder.listSets(token);
                if (m_print)
                    printResult("listSets(" + token + ")", data);
    ...
    private void doListRecordsOrIdentifiersTest(String from, String until, String prefix, String set, boolean identifiers) throws Exception
    {
        ...
        try
        {
            ...
            while (token != null)
            {
                ...
                if (m_print)
                    printResult("list" + which + "(null, null, null, null, " + token + ")", data);
                token = data.getResumptionToken();
            }
        }
        catch (ProtocolException e)
        {
            System.out.println("For from = " + from + ", until = " + until + ", prefix = " + prefix + ", set = " + set + ", List" + which
                    + " got protocol exception: " + e.getMessage());
        }
        finally
        {
            if (data != null)
