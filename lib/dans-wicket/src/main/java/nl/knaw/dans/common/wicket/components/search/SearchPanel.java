...
/**
 * This panel not only uses the SearchModel, but also updates it with every page refresh if it has become dirty. It uses an abstract method for this that needs
...
public abstract class SearchPanel extends BaseSearchPanel {
    ...
    /**
     * This method is called whenever the search model's search results need to be updated (when the search model is dirty). This may happen for example when a
     * sort method has been selected, a new result page is selected or when a facet has been clicked. The search can easily be implemented using the DANS
     * Commons search implementation, but one is of course free to use any method he/she finds suitable.
     * 
     * @param request
     *        the request
