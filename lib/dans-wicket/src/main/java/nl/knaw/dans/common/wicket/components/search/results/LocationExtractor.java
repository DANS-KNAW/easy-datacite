...
public class LocationExtractor {
    ...
    /**
     * @param dcCoverage
     *        Should be the dcCoverage from a DatasetSB
     * @return The location coordinates
     */
    public static LonLat getLocation(List<String> dcCoverage) {
        LonLat lonlat = null;
        // extract the RD point coordinates if available (No box yet!)
        if (dcCoverage != null) {
    ...
    private static LonLat getLocationFromEPSG4326(String[] coords) throws IllegalArgumentException {
        ...
        if (coords.length == 3) {
            ...
            try {
                ...
                throw new IllegalArgumentException("Unable to extract box coordinates from " + coords[1] + ", " + coords[2] + coords[3] + ", " + coords[4], e);
            }
        } else {
