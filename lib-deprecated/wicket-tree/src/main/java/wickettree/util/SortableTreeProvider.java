...
/**
 * Convenience implementation of a tree provider that can also act as a locator for a {@link SingleSortState} object.
 * 
 * @author Sven Meier
 */
public abstract class SortableTreeProvider<T> implements ISortableTreeProvider<T> {
    ...
    public final void setSortState(ISortState state) {
        if (!(state instanceof SingleSortState)) {
            throw new IllegalArgumentException("argument [state] must be an instance of SingleSortState, but it is [" + state.getClass().getName() + "]:["
                    + state.toString() + "]");
        }
