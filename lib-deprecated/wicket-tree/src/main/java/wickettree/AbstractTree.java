...
/**
 * Abstract base class for {@link NestedTree} and {@link TableTree}. Uses its model for storing the {@link State} of its nodes. Note that a tree has no notion
 * of a <em>selection</em>. Handling state of nodes besides expanse/collapse is irrelevant to a tree implementation.
 * 
...
public abstract class AbstractTree<T> extends Panel {
    ...
    /**
     * @return currently set item reuse strategy. Defaults to <code>DefaultItemReuseStrategy</code> if none was set.
    ...
    /**
     * Expand the given node, tries to update the affected branch if the change happens on an {@link AjaxRequestTarget}.
     * 
     * @see #getModelObject()
     * @see Set#add(Object)
     * @see #updateBranch(Object, AjaxRequestTarget)
    ...
    /**
     * Collapse the given node, tries to update the affected branch if the change happens on an {@link AjaxRequestTarget}.
     * 
     * @see #getModelObject()
    ...
    /**
     * Convenience method to update a single branch on an {@link AjaxRequestTarget}. Does nothing if the given node is currently not visible or target is
     * <code>null</code>. This default implementation adds this whole component for rendering.
     * 
     * @param t
     * @param target
     */
    ...
    /**
     * Convenience method to update a single node on an {@link AjaxRequestTarget}. Does nothing if the given node is currently not visible or target is
     * <code>null</code>.
     * 
     * @param t
     * @param target
    ...
    public void updateNode(T t, final AjaxRequestTarget target) {
        if (target != null) {
            ...
            visitChildren(Node.class, new IVisitor<Component>() {
                public Object component(Component node) {
                    if (model.equals(node.getDefaultModel())) {
