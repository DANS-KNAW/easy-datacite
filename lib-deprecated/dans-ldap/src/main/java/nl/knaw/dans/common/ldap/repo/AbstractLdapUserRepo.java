...
public abstract class AbstractLdapUserRepo<T extends User> extends AbstractGenericRepo<T> implements UserRepo<T> {
    ...
    /**
     * Note that {@link User.getPassword()} will not give the password from the repository after 'unmarshalling'. The user repository must be queried for this
     * because the password is never retrieved from the repository and the User object does not contain it.
     */
