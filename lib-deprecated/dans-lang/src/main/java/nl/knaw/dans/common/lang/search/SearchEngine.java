...
/**
 * The main search interface on which the three main operations of the search engine are defined. 1.
 * Search Searches through a specific search index and returns the found document in the form of search
 * beans or documents. 2. Index Adds or updates new or existing documents into a specific search index.
 * 3. Delete Deletes documents from a search index.
 * 
 * @author lobo
 */
...
{
    /**
     * Sets the current search bean factory. The search bean factory is responsible for the creation of
     * the search beans.
     */
    void setSearchBeanFactory(SearchBeanFactory seFactory);

    /**
     * Gets the current search bean factory. The search bean factory is responsible for the creation of
     * the search beans.
     */
    ...
    /**
     * Perform a search on the search engine and returns the search result in the form of document
     * objects.
     * 
     * @param request
     *        the search request
    ...
    /**
     * Perform a search on the search engine and returns the search result in the form of search beans.
     * Note: It is advisable to use the SearchRequest.setFilterBeans() method on the search request
     * before calling this method. If a found document cannot be converted to a search bean an exception
     * of some sort will be thrown..
     * 
     * @param request
     *        the search request
