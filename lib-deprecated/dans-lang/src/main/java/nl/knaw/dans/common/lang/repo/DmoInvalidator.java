...
/**
 * This class is responsible for keeping track of which data model object have been invalidated. It is a package level class that works in close cooporation
 * with AbstractDmoStore. It is not to be used by the public, instead the public currently use the DmoStore.
 * 
 * @author lobo
 */
abstract class DmoInvalidator {
    /**
     * A huge pool in which weak-references data model objects get stored and information about their invalidation status get stored.
     */
    ...
    /**
     * Implements a three-fold mechanism. First of all it checks if the object is known in the weak referenced pool of active objects in which the invalidation
     * status is actively kept. Then it checks the last modified cache to see if the object has been changed recently. If that is not possible, because the
     * object is older than the cache, then the repository is queried for the last modified date.
     */
    ...
    /**
     * Kept with each weak-referenced dmo in the pool. Stores a simple boolean on whether the dmo is invalidated.
     * 
     * @author lobo
     */
